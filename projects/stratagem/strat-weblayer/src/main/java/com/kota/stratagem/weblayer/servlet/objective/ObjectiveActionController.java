...
public class ObjectiveActionController extends HttpServlet implements ObjectiveParameter, ObjectiveAttribute {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		final ObjectiveRepresentor objective = new ObjectiveRepresentor("", "", 0, ObjectiveStatusRepresentor.PLANNED);
		this.forward(request, response, objective, false);
	}

	private void forward(final HttpServletRequest request, final HttpServletResponse response, final ObjectiveRepresentor objective, final boolean finishFlag)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if ((name == null) || "".equals(name)) {
				...
				request.setAttribute(ATTR_ERROR, "Objective name required");
				final ObjectiveRepresentor objective = new ObjectiveRepresentor(description, description, 5, status);
				this.forward(request, response, objective, false);
			} else {
				ObjectiveRepresentor objective = null;
				try {
					LOGGER.info(id == null ? "Create Objective : (" + name + ")" : "Update Objective : (" + id + ")");
					request.setAttribute(ATTR_SUCCESS, "Objective created succesfully!");
					objective = this.protocol.saveObjective(id, name, description, priority, status, null, null, null, null);
				} catch (final AdaptorException e) {
					LOGGER.error(e, e);
				}
				this.forward(request, response, objective, true);
			}
